...
#class MoERegressor(nn.Module): def __init__(self, input_dim=1024, select_dim=512, num_experts=6, top_k=3, shared_dim=32): super().__init__() # --- 模型参数 (保持不变) --- self.select_dim, self.num_experts, self.top_k, self.shared_dim = select_dim, num_experts, top_k, shared_dim self.mask_logits = nn.Parameter(torch.randn(num_experts, input_dim)) self.expert_fcs = nn.ModuleList([ nn.Sequential(nn.Linear(select_dim + shared_dim, 128), nn.ReLU(), nn.Linear(128, 1)) for _ in range(num_experts) ]) self.gate = nn.Linear(input_dim, num_experts) self.cross_attn = CrossAttention(query_dim=512, context_dim=512, heads=4, dim_head=16) self.shared_proj = nn.Linear(512, shared_dim) # --- 完全独立的分析部分 --- self.analysis_mode = True # 手动开启 self.analysis_file_path = "/home/data1/lq/SC-AGIQA-main/moe_analysis_results22.json" self._lock = threading.Lock() # 用于文件写入安全 self._is_first_write = True # 标记是否是第一次写入 def _append_to_file(self, data_list): """线程安全地将数据追加到JSON文件中""" with self._lock: # 如果是第一次写入，清空并创建文件 if self._is_first_write: print(f"[MoERegressor] First write, creating/clearing file: {self.analysis_file_path}") os.makedirs(os.path.dirname(self.analysis_file_path), exist_ok=True) with open(self.analysis_file_path, 'w') as f: json.dump(data_list, f, indent=4) self._is_first_write = False else: # 如果不是第一次，则以追加模式写入 # 这种追加方式比较复杂，为了简单和确保数据不丢失， # 我们采用一种更简单的方式：读-改-写 try: with open(self.analysis_file_path, 'r') as f: existing_data = json.load(f) except (FileNotFoundError, json.JSONDecodeError): existing_data = [] existing_data.extend(data_list) with open(self.analysis_file_path, 'w') as f: json.dump(existing_data, f, indent=4) def forward(self, x): # --- 核心计算 (完全不变) --- B, device = x.size(0), x.device x_sem, x_vis = x[:, :512], x[:, 512:] sem_token, vis_token = x_sem.unsqueeze(1), x_vis.unsqueeze(1) shared_token = self.cross_attn(sem_token, context=vis_token) shared_info = self.shared_proj(shared_token.squeeze(1)) gate_logits = self.gate(x) topk_scores, topk_indices = torch.topk(gate_logits, self.top_k, dim=1) topk_softmax = torch.softmax(topk_scores, dim=1) output = torch.zeros(B, 1, device=device) if self.analysis_mode: batch_records = [ {'topk_indices': topk_indices[i].cpu().tolist(), 'topk_weights': topk_softmax[i].cpu().tolist(), 'experts_details': []} for i in range(B) ] for i in range(self.top_k): expert_idx_batch, weight_batch = topk_indices[:, i], topk_softmax[:, i].unsqueeze(1) out_for_this_k = torch.zeros(B, 1, device=device) for expert_id in range(self.num_experts): mask = (expert_idx_batch == expert_id) if mask.any(): active_indices_in_batch = mask.nonzero(as_tuple=True)[0] x_sel, shared_sel = x[active_indices_in_batch], shared_info[active_indices_in_batch] mask_prob = torch.sigmoid(self.mask_logits[expert_id]) _, selected_feature_indices = torch.topk(mask_prob, self.select_dim) x_selected = x_sel[:, selected_feature_indices] expert_input = torch.cat([x_selected, shared_sel], dim=1) out_part = self.expert_fcs[expert_id](expert_input) out_for_this_k[active_indices_in_batch] = out_part.view(-1, 1).to(out_for_this_k.dtype) if self.analysis_mode: for j, batch_idx in enumerate(active_indices_in_batch): batch_records[batch_idx]['experts_details'].append({ 'expert_id': expert_id, 'is_top_k_position': i, 'weight': weight_batch[batch_idx].item(), 'raw_output': out_part[j].item(), 'selected_feature_indices': selected_feature_indices.cpu().tolist() }) output += out_for_this_k * weight_batch # --- !! 关键修改：直接在 forward 中调用写入 !! --- if self.analysis_mode: self._append_to_file(batch_records) return output
# import json
# import threading
# class MoERegressor(nn.Module):
#     def __init__(self, input_dim=1024, select_dim=512, num_experts=6, top_k=3, shared_dim=32,
#                  debug_mode=False):
#         super().__init__()
#         self.select_dim, self.num_experts, self.top_k, self.shared_dim = select_dim, num_experts, top_k, shared_dim
#         self.debug_mode = debug_mode  # 新增调试开关

#         # --- MoE 原有组件 ---
#         self.mask_logits = nn.Parameter(torch.randn(num_experts, input_dim))
#         self.expert_fcs = nn.ModuleList([
#             nn.Sequential(nn.Linear(select_dim + shared_dim, 128), nn.ReLU(), nn.Linear(128, 1))
#             for _ in range(num_experts)
#         ])
#         self.gate = nn.Linear(input_dim, num_experts)
#         self.cross_attn = CrossAttention(query_dim=512, context_dim=512, heads=4, dim_head=16)
...
#         self.analysis_file_path = "/home/data1/lq/SC-AGIQA-main/moe_analysis_results22.json"
#         self._lock = threading.Lock()
#         self._is_first_write = True

#     def _append_to_file(self, data_list):
#         with self._lock:
#             if self._is_first_write:
#                 print(f"[MoERegressor] First write, creating/clearing file: {self.analysis_file_path}")
#                 os.makedirs(os.path.dirname(self.analysis_file_path), exist_ok=True)
...
#     def forward(self, x):
#         # 如果开启 debug_mode，直接用简单 MLP 返回结果
#         if self.debug_mode:
#             return self.debug_mlp(x)

#         # --- MoE 原有逻辑 ---
#         B, device = x.size(0), x.device
#         x_sem, x_vis = x[:, :512], x[:, 512:]
#         sem_token, vis_token = x_sem.unsqueeze(1), x_vis.unsqueeze(1)
#         shared_token = self.cross_attn(sem_token, context=vis_token)
#         shared_info = self.shared_proj(shared_token.squeeze(1))
#         gate_logits = self.gate(x)
#         topk_scores, topk_indices = torch.topk(gate_logits, self.top_k, dim=1)
#         topk_softmax = torch.softmax(topk_scores, dim=1)
#         output = torch.zeros(B, 1, device=device)

#         if self.analysis_mode:
#             batch_records = [
#                 {'topk_indices': topk_indices[i].cpu().tolist(), 'topk_weights': topk_softmax[i].cpu().tolist(), 'experts_details': []}
#                 for i in range(B)
#             ]

#         for i in range(self.top_k):
#             expert_idx_batch, weight_batch = topk_indices[:, i], topk_softmax[:, i].unsqueeze(1)
#             out_for_this_k = torch.zeros(B, 1, device=device)
#             for expert_id in range(self.num_experts):
#                 mask = (expert_idx_batch == expert_id)
#                 if mask.any():
#                     active_indices_in_batch = mask.nonzero(as_tuple=True)[0]
#                     x_sel, shared_sel = x[active_indices_in_batch], shared_info[active_indices_in_batch]
#                     mask_prob = torch.sigmoid(self.mask_logits[expert_id])
#                     _, selected_feature_indices = torch.topk(mask_prob, self.select_dim)
#                     x_selected = x_sel[:, selected_feature_indices]
#                     expert_input = torch.cat([x_selected, shared_sel], dim=1)
#                     out_part = self.expert_fcs[expert_id](expert_input)
#                     out_for_this_k[active_indices_in_batch] = out_part.view(-1, 1).to(out_for_this_k.dtype)
#                     if self.analysis_mode:
#                         for j, batch_idx in enumerate(active_indices_in_batch):
#                             batch_records[batch_idx]['experts_details'].append({
#                                 'expert_id': expert_id, 'is_top_k_position': i,
#                                 'weight': weight_batch[batch_idx].item(), 'raw_output': out_part[j].item(),
#                                 'selected_feature_indices': selected_feature_indices.cpu().tolist()
#                             })
#             output += out_for_this_k * weight_batch

#         if self.analysis_mode:
#             self._append_to_file(batch_records)
...
class IQARegression(nn.Module):
    def __init__(self, inchannels=768, outchannels=512):
        ...
        self.experts_new= MoERegressor(num_experts=4,select_dim=512,top_k=3)  #,debug_mode=False
